...
export const ROUTER_SOURCE = 'pharma-product-facts'
...
const PRODUCT_ENTITIES = [
  '德瑞妥', '得佑', '天韵', '甘美', '甘平', '贝乐林',
  '妥洛特罗', '利拉鲁肽', '利拉鲁泰', '链霉蛋白酶',
  '异甘草酸镁', '甘草酸二铵', '甘草酸二胺', '多黏菌素', '多粘菌素',
] as const

const LABEL_FACT_TERMS = [
  '适应症', '适应证', '规格', '剂型', '成分', '辅料', '储存', '贮藏', '有效期',
  '半衰期', '达峰时间', '药代动力学', '作用机制', '作用机理', '药理作用',
  '用法用量', '说明书上', '说明书里', '说明书怎么', '说明书是怎么',
] as const

const PRODUCT_SCOPED_TERMS = [
  '是否获批', '获批用于', '获批适应症', '获批适应证', '超说明书', '超适应症',
  '不良反应', '禁忌', '慎用', '警示', '注意事项', '特殊人群',
  '妊娠', '哺乳', '孕妇', '孕产妇', '儿童', '老年',
] as const

const HCP_FOCUS_INTENTS = [
  '医生在临床使用中主要关注', '医生主要关注哪些关键问题', '医生关注的关键问题',
  '处方时的医生考量因素', '处方时医生考量因素', '处方时医生需要考虑',
  '医生处方考量', '临床应用要点',
] as const

const CLINICAL_EXCLUSIONS = [
  '这个患者', '该患者', '患者用了', '患者使用', '患者用药', '我的情况', '本人',
  '我正在吃', '我在吃', '我正在用', '我在用', '我准备吃', '我准备用',
  '按体重', '按肌酐', '肌酐清除率', 'egfr', 'crcl', 'bsa',
  '剂量计算', '剂量调整', '减量', '调量', '配液', '复溶', '稀释', '雾化', '贴敷',
  '漏服', '漏打', '补打', '漏用', '漏吃', '不良反应怎么', '副作用怎么办',
  '怎么处置', '如何处置', '处理不良反应', '用药后出现', '使用后出现',
  '联用', '联合用药', '配伍', '相互作用', '肝功能不全', '肾功能不全', '肝肾功能不全',
] as const

const COMPARISON_EXCLUSIONS = [
  '竞品', '原研', '同类', '头对头', 'head-to-head', '对比', '相比', '相较',
  '优劣', '谁更好', '哪家好', '差异化',
] as const

const COMMERCIAL_EXCLUSIONS = [
  '推广', '卖点', 'fabe', '话术', '销售', '物料', '邀请函', '拜访',
  '处方率', '处方量', '提单', '上量', '让医生开', '让他开',
] as const

const METADATA_EXCLUSIONS = [
  '受理号', '批准文号', '注册分类', '生产企业', '生产厂家', '持有人',
  '下载说明书', '爬说明书', '抓取说明书', '说明书 pdf',
] as const

const EVIDENCE_EXCLUSIONS = [
  'pmid', 'doi', 'rct', '文献', '论文', '研究证据', '临床研究', '临床试验',
  '系统综述', '系统评价', 'meta分析', '荟萃分析', '指南', '共识', '循证',
] as const
...
function hasExcludedIntent(text: string): boolean {
  return PATIENT_PARAMETER.test(text)
    || includesAny(text, CLINICAL_EXCLUSIONS)
    || includesAny(text, COMPARISON_EXCLUSIONS)
    || includesAny(text, COMMERCIAL_EXCLUSIONS)
    || includesAny(text, METADATA_EXCLUSIONS)
    || includesAny(text, EVIDENCE_EXCLUSIONS)
}
 ...
 */
export function matchesPharmaProductFacts(value: string): boolean {
 ...
 */
function matchesPharmaProductFactsMessages(messages: readonly UserMessage[]): boolean {
  return messages.some(message => message.source.kind === 'user'
    && matchesPharmaProductFacts(textOf(message)))
}
 ...
 */
export function registerPharmaProductFactsRouter(ctx: Context): void {
