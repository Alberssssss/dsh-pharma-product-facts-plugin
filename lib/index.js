...
*/
function validateRedundantHcpBoundary(boundary, focus, store, scope, product) {
	if (boundary === void 0) return;
	if (boundary.approval_status !== "listed") throw new Error("hcp_focus_card accepts only a redundant listed label_boundary");
	const questionedUse = requiredLine(boundary.questioned_use, "label_boundary.questioned_use", 100);
	const scopeQuote = requiredLine(boundary.scope_quote, "label_boundary.scope_quote", 4e3);
	if (!focus.some((item) => item.evidence_id === boundary.evidence_id && line(item.quote) === scopeQuote)) throw new Error("hcp_focus_card label_boundary must duplicate one clinical_focus quote and evidence_id");
	if (!evidenceFor(store, scope, boundary.evidence_id, product, scopeQuote).quote.includes(questionedUse)) throw new Error("hcp_focus_card listed use is not present in the duplicated clinical_focus quote");
}
...
function hcpFocusAnswer(input, store, scope) {
	const { product, title } = identity(input);
	if ((input.facts || []).length > 0) throw new Error("hcp_focus_card does not accept facts");
...
function registerPharmaProductFactsTools(ctx, config, options = {}) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			product: {
				type: "string",
				description: "Required outside boundary_or_failure; exact product used to fetch evidence."
			},
			title: {
				type: "string",
				description: "Optional public title containing the exact product identity."
			},
			facts: {
				type: "array",
				items: factSchema,
